...
int main(int argc, char **argv)
{
	if (argc == 2 && (
		...
		cout << "Usage: console-interaction-tester.exe SOME_BIN_TEST_SUBJECT TEST_INPUT EXPECTED_OUTPUT";
		return 0;
	}

	setlocale( LC_ALL, (const char *)LOCALE_SYSTEM_DEFAULT);
	...
	if (
		!DuplicateHandle(
			GetCurrentProcess(), hOutputReadTmp, GetCurrentProcess(), &hOutputRead, // Address of new handle.
...
void PrepAndLaunchRedirectedChild(HANDLE hChildStdOut, HANDLE hChildStdIn, HANDLE hChildStdErr)
{
	...
	// confusion.
	if (!CreateProcess(NULL, "console-interaction-tester.exe", NULL, NULL, TRUE, CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi))
